...
| 19 ↑ | Advanced analytics | **MOSTLY_COMPLETE** | 72 | `charts.js`, forecasting, anomaly detection, trends, actual/projected labels; **a month in progress is no longer compared with one that finished** — `comparePeriods` measured the days elapsed against the whole of last month, so a household spending the same every day was told its spending was 94% below last month on the 2nd, in the colour reserved for good news, and the assistant wrote it into a sentence. `unusual.js` states the rule, `runway.js` drops the partial month and the CFO screen shows it apart and marked; the two screens a household actually opens did none of it (`docs/THREE_DAYS_IN_EVERYTHING_HAS_FALLEN.md`) · `wired:js/domain/finance.js#previousToDate` | No ML; forecasts are rule-based. The mid-month comparison is now between two equal spans of real records, which is honest rather than exact: rent falling on the 1st of one month and the 2nd of the next still crosses the boundary, and the caption names the span rather than claiming precision | Low |
