...
| 23 ↑ | Android companion | **PARTIALLY_COMPLETE** | 64 | Capacitor 8.5.0; **debug APK builds in CI**; back button, Filesystem, Share; `allowBackup` off; coarse and fine location; **a first-party SMS plugin**, telephony declared non-required | No background location or geofencing, no screen time, no camera; **never run on a real device** — it compiles in CI and nothing here has installed it; the SMS permission makes this a sideload build | Medium |
...
| 25 ↑ | Internationalisation | **PARTIALLY_COMPLETE** | 45 | `js/core/locale.js` merged in `e5b45df`; schema labels and dates route through it; a translation that drops a placeholder is **refused**; `tools/strings.mjs` measures the rest | **One language.** 3,487<!--live:unroutedStrings--> strings are still written into the source, and nothing has been translated | Low |
