...
| Authorization / RBAC | **working, server-authoritative** — `js/security/rbac.js` plus a generated `apps-script/Policy.gs` covering 47<!--live:entities--> entities |
...
| SMS Security | **working** — the OTP gate runs *before any field is read*, and nothing is stored. Proved across all 54<!--live:stores--> stores, not the one table it would most likely land in |
