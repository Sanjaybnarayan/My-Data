...
| **#258** | The same three faults as #244, one level further in and reachable by any authenticated member. `schema`, `push` and `audit` each walked a list out of the payload reading fields off every element, having never asked whether it was a list — `{"changes":[null]}` threw, and `{"length":3}` got past the `.length` guard each of them had. On `push` the throw discarded every good change batched beside the bad one. | `requestList` tests `Array.isArray`; `isRecordLike` gates each element. `push` rejects the row and applies its neighbours; `schema` and `audit` refuse the batch, because a hole in an append-only log is worse than a refusal the client can retry. Found by pointing the same fuzz suite past the front door — seven of 288 payloads mishandled, 281 held. |
