...
| --- | --- |
| **Foundation** | Storage adapter with IndexedDB and in-memory implementations of one interface; the schema registry that forty<!--live:entities--> entities, their stores, indexes, forms, validation, encryption, Sheets tabs and the assistant's vocabulary are all derived from; single-transaction writes; audit trail |
...
| **Modules** | Nineteen<!--live:modules-->, over forty<!--live:entities--> entities. Ten are the same file reading the schema; the other nine have a screen of their own — dashboard, finance, investments, documents, family, calendar, identity, reports and settings — as does the assistant, which is a route rather than a module |
