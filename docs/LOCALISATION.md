...
| `js/locale/en.js` | The authoritative catalogue. 38<!--live:localeKeys--> keys today |
...
**3,270<!--live:unroutedStrings--> English strings, across 136<!--live:unroutedFiles--> files.** That is the measured count from
...
- 38<!--live:localeKeys--> UI message keys — dates, the generic record screens, the language card
- 681<!--live:labelKeys--> schema label keys — 21<!--live:modules--> modules, 47<!--live:entities--> entities in two forms each, 566<!--live:fields--> fields

681<!--live:labelKeys--> of those are derived from the schema by `labelKeys()`, so a new entity is
...
and machine-translating 3,270<!--live:unroutedStrings--> strings of Indian financial and legal vocabulary
...
1. `js/locale/en.js` — 38<!--live:localeKeys--> strings, whole sentences, with `{placeholders}`
   intact. Every placeholder must survive or the line is refused.
2. The 681<!--live:labelKeys--> label keys from `labelKeys()` — entity names, field names, module
   names.
