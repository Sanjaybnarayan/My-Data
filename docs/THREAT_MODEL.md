...
| **T2.4** A compromised dependency in either of the APK's two supply chains | Eight `@capacitor/*` npm packages, and five Maven coordinates — `androidx.appcompat`, `coordinatorlayout`, `core-splashscreen`, `biometric`, `com.google.mlkit:text-recognition` — plus everything those pull transitively. All compiled into the APK | Low per-package, and there are far more than thirteen once transitives are counted | Arbitrary code inside the app's own process, with the data key in scope | **Pinning is real, and this row said otherwise.** `package-lock.json` is committed, pins every runtime package to an exact version with a sha512 integrity hash, and all three CI jobs install with `npm ci`, which fails if the lock and the manifest disagree; the caret is only what `package.json` *declares*. Every Maven version in `android/variables.gradle` is exact — no range, nothing Gradle resolves at build time — and `tools/supply-chain.mjs` fails the build if that stops being true. **`npm audit --omit=dev` now runs in CI**: 0 findings across 11 runtime packages | **The npm half is scanned; the Maven half is scanned by nothing.** Querying an advisory service for `androidx.biometric:biometric:1.1.0` needs a host the build environment cannot reach, so no such check was written rather than one being written and never watched to fail. Provenance is still unverified in both chains, and the listed coordinates are direct declarations — what ships is those plus whatever they chose. **The row previously read "no pinning beyond the caret", which was wrong**, and overstating a gap costs the same credibility as understating one |
...
| **T4.7** A row that is not what its column says takes a screen down | A `message` row whose `body` is `null`, a number, or an envelope whose `keys` is not a list. It arrives through a pull like any other row — from a misbehaving deployment, another device, or a truncated write | **Medium** — nothing between here and the store validates a synced row's shape, and one bad byte in transit does it as readily as an attacker | The Chat screen shows **nothing at all**, not one blank line: `read` threw and the conversation list threw with it. The household cannot reach into the store to delete the row that is doing it, so it does not clear on its own | `ChatService` refuses a parse result that is not an object with a list of wraps, and `sealedTo`, `addressedTo` and `openBytes` answer for anything rather than throwing — the same guard, for the same reason, as the one `doPost` grew for a request body of `null` | **This is one screen's worth of the general problem.** `applyRemote` still validates no synced row against its schema, and the same shape of row could be waiting behind any other list that reads a field off one without checking. What is fixed here is the screen that was measured, not the class |
| **T4.8** A list in a request body that is not a list of records takes the deployment down with it | `push`, `schema` and `audit` each take an array out of the payload and walk it reading fields off each element. `{"changes":[null]}` from any authenticated member reached `changes[c].store`; `{"length":3}` reached the same place past a `.length` guard, because an array-like object answers every index with `undefined` | **Medium** — this is the same class as T4.7 arriving from the other direction, and a truncated or half-written outbox entry produces it as readily as somebody trying | The three faults `doPost` already names for a body of `null`, reproduced one level in: a V8 internal message returned to the caller, a **500** saying the deployment had broken when the request was malformed, and **`retryable: true`**, so the sender's outbox resends a permanently invalid request. On `push` it also discards every good change batched beside the bad one | `requestList` tests `Array.isArray` rather than truthiness, and `isRecordLike` gates every element. `push` rejects the bad row and applies its neighbours — its own stated rule, that one change a child may not make should not throw away the fourteen they may. `schema` and `audit` refuse the batch with a 400, because a hole in an append-only log is worse than a refusal the client can retry. **LIST-02** adds the fourth: `manageMembers` walks `payload.emails`, is in `Code.gs`, and was not among the three — sent a string it iterated the characters, wrote the empty result over the household's access list, and answered `ok: true` | **Inert until the Apps Script is redeployed**, like every other change under `apps-script/`. The sentence that stood here — *"this closes three measured handlers, not the class"* — was right, and the fourth handler was sitting unmeasured while it said so. So the class is swept now rather than counted: `tests/fuzz.test.mjs` drives **every** action `served()` names, derived from the backend's own dispatch, against nine hostile payloads, asserting what the file already claims — never a 5xx, never a runtime message quoted back, never `retryable` on a 4xx. A new action joins the sweep by existing. What that does *not* reach is a field holding a plausible wrong value rather than a wrong shape, which is T4.7's residual and still the honest statement of it |
