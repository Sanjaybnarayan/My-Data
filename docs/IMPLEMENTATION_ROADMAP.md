...
| **6** | **SMS Intelligence** | **started, having been skipped without acknowledgement** — reader, 25 categories, OTP gate that runs before any field is read, dedupe and statement reconciliation (`docs/SMS_INTELLIGENCE.md`). **No `SMSMessage`/`SMSEvent`/`SMSSource`/`SMSProcessingRecord` entities, nothing persisted, no Gmail↔SMS three-way link.** See below for how the phase was lost |
