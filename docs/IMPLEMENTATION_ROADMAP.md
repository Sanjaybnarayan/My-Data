...
| 10 | Insurance, vehicles, fuel, property, tenants, purchases, warranty, subscriptions, travel | **entities exist, intelligence mostly does not** — mileage is measured between full tanks and refuses everything it cannot bracket (`docs/FUEL_MILEAGE.md`); **no fuel-bill reading, no warranty extraction, no travel**. Rent *is* tracked, and a payment two lettings could claim is now attributed to neither rather than receipted twice (`docs/RENT_ATTRIBUTION.md`); **no tenant entity, no lease history, no arrears** |
...
| 17 | Knowledge graph, universal search, family timeline, what changed | search exists; every record says what has happened to it (`docs/RECORD_HISTORY.md`), and the activity feed groups the log into things that happened, names the record and marks what is new (`docs/ACTIVITY_STORIES.md`). connections on every record, in both directions, drawn only where one record stores another's id (`docs/CONNECTIONS.md`) — which turned up `referencedBy` missing all seventeen attachment edges. **No timeline screen of its own — the feed is a dashboard widget eight stories deep** |
